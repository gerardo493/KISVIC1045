...
def mostrar_facturas():
    ...
    cliente_search = request.args.get('cliente', '').strip()
    fecha_desde = request.args.get('fecha_desde', '')
    fecha_hasta = request.args.get('fecha_hasta', '')
    ordenar = request.args.get('ordenar', 'fecha_desc')
    filtro_estado = request.args.get('estado', 'todas')
    if search or cliente_search or fecha_desde or fecha_hasta or filtro_estado != 'todas':
        filtered_facturas = {}
        for id, factura in facturas.items():
            if search and search.lower() not in str(factura.get('numero', '')).lower():
                continue
            if cliente_search:
                cliente_id = factura.get('cliente_id', '')
                cliente = clientes.get(cliente_id, {})
                cliente_nombre = cliente.get('nombre', '').lower()
                cliente_rif = cliente_id.lower()
                if cliente_search.lower() not in cliente_nombre and cliente_search.lower() not in cliente_rif:
                    continue
            if fecha_desde or fecha_hasta:
                try:
                    factura_fecha = datetime.strptime(factura.get('fecha', ''), '%Y-%m-%d')
                    if fecha_desde:
                        desde_fecha = datetime.strptime(fecha_desde, '%Y-%m-%d')
                        if factura_fecha < desde_fecha:
                            continue
                    if fecha_hasta:
                        hasta_fecha = datetime.strptime(fecha_hasta, '%Y-%m-%d')
                        if factura_fecha > hasta_fecha:
                            continue
                except (ValueError, TypeError):
                    continue
            if filtro_estado != 'todas' and factura.get('estado', 'pendiente') != filtro_estado:
                continue
            filtered_facturas[id] = factura
        facturas = filtered_facturas
    facturas_list = list(facturas.items())
    def get_cliente_nombre(factura):
        cliente_id = factura.get('cliente_id', '')
        cliente = clientes.get(cliente_id, {})
        return cliente.get('nombre', '').lower()
    def get_estado_orden(factura):
        ...
        if estado == 'cobrada':
            ...
            return 2
    def get_fecha(factura):
        try:
            return datetime.strptime(factura.get('fecha', ''), '%Y-%m-%d')
        except Exception:
            return datetime.min
    def get_numero(factura):
        try:
            return int(factura.get('numero', 0))
        except Exception:
            return 0
    def get_total_usd(factura):
        try:
            return float(factura.get('total_usd', 0))
        except Exception:
            return 0.0
    def get_total_bs(factura):
        try:
            return float(factura.get('total_bs', 0))
        except Exception:
            return 0.0
    if ordenar == 'fecha_desc':
        facturas_list.sort(key=lambda x: get_fecha(x[1]), reverse=True)
    elif ordenar == 'fecha_asc':
        facturas_list.sort(key=lambda x: get_fecha(x[1]))
    elif ordenar == 'numero_desc':
        facturas_list.sort(key=lambda x: get_numero(x[1]), reverse=True)
    elif ordenar == 'numero_asc':
        facturas_list.sort(key=lambda x: get_numero(x[1]))
    elif ordenar == 'total_usd_desc':
        facturas_list.sort(key=lambda x: get_total_usd(x[1]), reverse=True)
    elif ordenar == 'total_usd_asc':
        facturas_list.sort(key=lambda x: get_total_usd(x[1]))
    elif ordenar == 'total_bs_desc':
        facturas_list.sort(key=lambda x: get_total_bs(x[1]), reverse=True)
    elif ordenar == 'total_bs_asc':
        facturas_list.sort(key=lambda x: get_total_bs(x[1]))
    elif ordenar == 'cliente_asc':
        facturas_list.sort(key=lambda x: get_cliente_nombre(x[1]))
    elif ordenar == 'cliente_desc':
        facturas_list.sort(key=lambda x: get_cliente_nombre(x[1]), reverse=True)
    elif ordenar == 'estado_asc':
        facturas_list.sort(key=lambda x: get_estado_orden(x[1]))
    elif ordenar == 'estado_desc':
        facturas_list.sort(key=lambda x: get_estado_orden(x[1]), reverse=True)
    facturas = dict(facturas_list)
    return render_template('facturas.html', facturas=facturas, clientes=clientes, filtro_estado=filtro_estado)
...
def editar_factura(id):
    ...
    if request.method == 'POST':
        try:
            ...
            actualizar_estados_facturas()  # Recalcula estados automáticamente
...
def ajustar_stock():
    ...
    # Filtros y orden
    q = request.args.get('q', '').strip().lower()
    filtro_categoria = request.args.get('categoria', None)
    filtro_orden = request.args.get('orden', 'nombre')
    ...
    if filtro_categoria:
        inventario = {k: v for k, v in inventario.items() if v.get('categoria') == filtro_categoria}
    # Ordenar
    if filtro_orden == 'nombre':
    ...
    if request.method == 'POST':
            ...
            if producto_id in inventario:
                ...
                if tipo_ajuste == 'entrada':
                    ...
                    if cantidad > cantidad_actual:
                        flash(f'No hay suficiente stock disponible para {producto["nombre"]}', 'error')
                        continue
                    producto['cantidad'] -= cantidad
        ...
        registrar_bitacora(usuario, 'ajuste_stock', f"Ajuste masivo: {tipo_ajuste} de {cantidad} unidades para {len(productos)} producto(s). Motivo: {motivo}")
        flash('Ajuste masivo realizado correctamente', 'success')
        return redirect(url_for('ajustar_stock'))
    return render_template('ajustar_stock.html', inventario=inventario, categorias=categorias, q=q, filtro_categoria=filtro_categoria, filtro_orden=filtro_orden)
...
def reporte_clientes():
    try:
        ...
        for factura in facturas.values():
            try:
                t_bcv = float(factura.get('tasa_bcv', tasa_bcv))
                total_usd = float(factura.get('total_usd', 0))
                total_facturado_usd += total_usd
                total_facturado_bs += total_usd * t_bcv
                if 'pagos' in factura and factura['pagos']:
                    for pago in factura['pagos']:
                        try:
                            monto = float(str(pago.get('monto', 0)).replace('$', '').replace(',', ''))
                            moneda = pago.get('moneda', 'USD').upper()
                            if moneda == 'BS':
                                total_abonado_bs += monto
                                total_abonado_usd += monto / t_bcv
                            else:
                                total_abonado_usd += monto
                                total_abonado_bs += monto * t_bcv
                        except Exception:
                            continue
            except Exception:
                continue
        ...
        total_por_cobrar_usd = round(total_facturado_usd - total_abonado_usd, 2)
        total_por_cobrar_bs = round(total_facturado_bs - total_abonado_bs, 2)
        ...
        for factura in facturas.values():
            try:
                ...
                if not cliente_id:
                    continue
                cliente = clientes.get(cliente_id, {})
                nombre_cliente = cliente.get('nombre', 'Cliente Desconocido')
                ...
                if 'pagos' in factura and factura['pagos']:
                    for pago in factura['pagos']:
                        try:
                            ...
                            t_bcv = float(factura.get('tasa_bcv', tasa_bcv))
                            if moneda == 'BS':
                                pagos_factura += monto / t_bcv
                            else:
                                pagos_factura += monto
                        except Exception:
                            continue
                saldo_pendiente = total_factura - pagos_factura
                # Top clientes
                ...
                'total_facturas': 0,
                'total_compras': 0,
                        'ultima_compra': fecha_factura,
                        'id': cliente_id,
                        'email': email,
                        'telefono': telefono
                    }
                clientes_stats[cliente_id]['total_facturas'] += 1
                clientes_stats[cliente_id]['total_compras'] += total_factura
                if fecha_factura and (not clientes_stats[cliente_id]['ultima_compra'] or fecha_factura > clientes_stats[cliente_id]['ultima_compra']):
                    clientes_stats[cliente_id]['ultima_compra'] = fecha_factura
                # Top deudores
                if saldo_pendiente > 0:
                    if cliente_id not in deudores_stats:
                        deudores_stats[cliente_id] = {
                            'nombre': nombre_cliente,
                'total_facturado': 0,
                'total_abonado': 0,
                            'total_por_cobrar': 0,
                    ...
                    deudores_stats[cliente_id]['total_facturado'] += total_factura
                    deudores_stats[cliente_id]['total_abonado'] += pagos_factura
                    deudores_stats[cliente_id]['total_por_cobrar'] += saldo_pendiente
            except Exception:
                continue
        top_clientes = sorted(clientes_stats.values(), key=lambda x: x['total_compras'], reverse=True)[:10]
        peores_clientes = sorted(deudores_stats.values(), key=lambda x: x['total_por_cobrar'], reverse=True)[:5]
                ...
                for i in range(len(productos)):
                    ...
                    if prod_id in inventario:
                        ...
                        try:
                            ...
                            productos_stats[prod_id]['valor'] += cantidad * precio
                        except Exception:
                            continue
            except Exception:
                continue
        top_productos = sorted(productos_stats.values(), key=lambda x: x['valor'], reverse=True)[:10]
        promedio_por_factura = round(total_facturado_usd / total_facturas, 2) if total_facturas > 0 else 0
        ...
        return f"Error en el reporte de clientes: {str(e)}", 500
    return "Error inesperado en el reporte de clientes", 500
...
def actualizar_estados_facturas():
    ...
    for id, factura in facturas.items():
        total_abonado = 0
        t_bcv = float(factura.get('tasa_bcv', 1))
        if 'pagos' in factura and factura['pagos']:
            for pago in factura['pagos']:
                try:
                    monto = float(str(pago.get('monto', 0)).replace('$', '').replace(',', ''))
                    moneda = pago.get('moneda', 'USD').upper() if 'moneda' in pago else 'USD'
                    if moneda == 'BS':
                        total_abonado += monto / t_bcv if t_bcv else 0
                    else:
                        total_abonado += monto
                except Exception:
                    continue
        total_abonado = round(total_abonado, 2)
        total_factura = round(float(factura.get('total_usd', 0)), 2)
        saldo_pendiente = round(total_factura - total_abonado, 2)

        # Usar un umbral muy pequeño para considerar el saldo como cero
        if abs(saldo_pendiente) < 0.0001 and total_factura > 0:
            factura['estado'] = 'cobrada'
...
@app.route('/facturas/recalcular-estados')
@login_required
def recalcular_estados_facturas():
    actualizar_estados_facturas()
    flash('Estados de facturas recalculados correctamente.', 'success')
    return redirect(url_for('mostrar_facturas'))
...
def mostrar_cuentas_por_cobrar():
    ...
    anios_disponibles = sorted({datetime.strptime(f.get('fecha', ''), '%Y-%m-%d').year for f in facturas.values() if f.get('fecha', '')})
    ...
    cuentas = {}
    top_deudores = []
    estados_count = {'por_cobrar': 0, 'abonada': 0, 'cobrada': 0}

    # Procesar todas las facturas
    for id, factura in facturas.items():
        # Filtrar por mes y año si corresponde
        if mes_seleccionado or anio_seleccionado:
            try:
                fecha_factura = datetime.strptime(factura.get('fecha', ''), '%Y-%m-%d')
                if mes_seleccionado and fecha_factura.month != mes_seleccionado:
                    continue
                if anio_seleccionado and fecha_factura.year != anio_seleccionado:
                    continue
            except Exception:
                continue
        ...
        try:
            ...
            saldo_pendiente = float(factura.get('saldo_pendiente', total_factura))
            estado = factura.get('estado', 'pendiente')
            t_bcv = float(factura.get('tasa_bcv', tasa_bcv))

            # Actualizar contadores de estado
            if estado == 'cobrada':
                estados_count['cobrada'] += 1
            elif estado == 'abonada':
                estados_count['abonada'] += 1
            else:
                estados_count['por_cobrar'] += 1

            # Actualizar totales globales
            total_facturado_usd += total_factura
            total_facturado_bs += total_factura * t_bcv
            total_abonado_usd += total_abonado
            total_abonado_bs += total_abonado * t_bcv

            # Si la factura coincide con el filtro, agregarla a cuentas
            if filtro == 'todas' or estado == filtro or (filtro == 'por_cobrar' and estado in ['pendiente', 'por_cobrar']):
                cliente_id = factura.get('cliente_id')
                cuentas[id] = {
                    'numero': factura.get('numero', ''),
                    'cliente_nombre': clientes.get(cliente_id, {}).get('nombre', 'Cliente no encontrado'),
            ...
            print(f"Error procesando factura {id}: {e}")
            continue

    # Calcular top deudores o mejores pagadores según el filtro
    if filtro == 'cobrada':
        # Para facturas cobradas, mostrar los mejores pagadores
        clientes_cobrados = {}
        for id, factura in facturas.items():
            if factura.get('estado') == 'cobrada':
                cliente_id = factura.get('cliente_id')
                if cliente_id:
                    if cliente_id not in clientes_cobrados:
                        clientes_cobrados[cliente_id] = {
                            'cliente': clientes.get(cliente_id, {}).get('nombre', 'Cliente no encontrado'),
                            'total_facturado': 0,
                            'abonado_usd': 0,
                            'facturas': 0
                        }
                    clientes_cobrados[cliente_id]['total_facturado'] += float(factura.get('total_usd', 0))
                    clientes_cobrados[cliente_id]['abonado_usd'] += float(factura.get('total_abonado', 0))
                    clientes_cobrados[cliente_id]['facturas'] += 1
        top_deudores = sorted(
            clientes_cobrados.values(),
            key=lambda x: x['abonado_usd'],
            reverse=True
        )[:5]
    elif filtro == 'abonada':
        # Para facturas abonadas, mostrar los mejores abonadores
        clientes_abonados = {}
        for id, factura in facturas.items():
            if factura.get('estado') == 'abonada':
                cliente_id = factura.get('cliente_id')
                if cliente_id:
                    if cliente_id not in clientes_abonados:
                        clientes_abonados[cliente_id] = {
                            'cliente': clientes.get(cliente_id, {}).get('nombre', 'Cliente no encontrado'),
                            'abonado_usd': 0,
                            'total_facturado': 0,
                            'facturas': 0
                        }
                    clientes_abonados[cliente_id]['abonado_usd'] += float(factura.get('total_abonado', 0))
                    clientes_abonados[cliente_id]['total_facturado'] += float(factura.get('total_usd', 0))
                    clientes_abonados[cliente_id]['facturas'] += 1
        top_deudores = sorted(
            clientes_abonados.values(),
            key=lambda x: x['abonado_usd'],
            reverse=True
        )[:10]
    else:
        # Para otros filtros, mostrar los deudores
        deudores = {}
        for id, factura in facturas.items():
            if factura.get('estado') != 'cobrada':
                cliente_id = factura.get('cliente_id')
                if cliente_id:
                    if cliente_id not in deudores:
                        deudores[cliente_id] = {
                            'cliente': clientes.get(cliente_id, {}).get('nombre', 'Cliente no encontrado'),
                            'monto': 0
                        }
                    deudores[cliente_id]['monto'] += float(factura.get('saldo_pendiente', 0))
        top_deudores = sorted(
            deudores.values(),
    ...
    grafica_barras = {
        'labels': ['Total Facturado', 'Total Abonado', 'Total por Cobrar'],
        'data': [total_facturado_usd, total_abonado_usd, total_facturado_usd - total_abonado_usd],
        'facturas': [estados_count['por_cobrar'], estados_count['abonada'], estados_count['cobrada']]
    }

    grafica_pastel = {
        'labels': ['Por Cobrar', 'Abonadas', 'Cobradas'],
        'data': [estados_count['por_cobrar'], estados_count['abonada'], estados_count['cobrada']]
    }
    ...
    return render_template('reporte_cuentas_por_cobrar.html',
                         ...
                         total_abonado_usd=total_abonado_usd,
                         total_abonado_bs=total_abonado_bs,
                         total_por_cobrar_usd=total_facturado_usd - total_abonado_usd,
                         total_por_cobrar_bs=total_facturado_bs - total_abonado_bs,
                         cantidad_facturas=len(cuentas),
    ...
    for f in facturas.values():
        if 'pagos' in f and f['pagos']:
            for pago in f['pagos']:
                ...
                if fecha_pago:
                    try:
                        dt = datetime.strptime(fecha_pago[:10], '%Y-%m-%d')
                        anios_disponibles.add(dt.year)
                    except Exception:
                        continue
        ...
        if 'pagos' in f and f['pagos']:
            for pago in f['pagos']:
                ...
                if fecha_pago:
                    try:
                        dt = datetime.strptime(fecha_pago[:10], '%Y-%m-%d')
                        if mes_seleccionado and dt.month != mes_seleccionado:
                            continue
                        if anio_seleccionado and dt.year != anio_seleccionado:
                            continue
                    except Exception:
                        continue
...
def reporte_facturas():
    ...
    for factura in facturas.values():
        ...
        if filtro_cliente and str(factura['cliente_id']) != filtro_cliente:
            continue
        # Usar el estado guardado, no recalcular
        total_abonado = float(factura.get('total_abonado', 0))
        total_factura = float(factura.get('total_usd', 0))
        factura['total_abonado'] = total_abonado
...
@app.route('/facturas/<id>/registrar_pago', methods=['GET', 'POST'])
@login_required
def registrar_pago(id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Actualizar estado de la factura
            if total_abonado >= total_factura:
                saldo_pendiente = 0
                factura['estado'] = 'cobrada'
            elif total_abonado > 0:
                factura['estado'] = 'abonada'
    ...
    return render_template('registrar_pago.html',
                         ...
                         tasa_bcv=factura.get('tasa_bcv', 1))
    ...
    try:
        ...
        for i, pago in enumerate(pagos):
            if str(pago.get('id', '')) == str(pago_id):
                ...
                # Actualizar estado de la factura
                if factura['total_abonado'] >= factura.get('total_usd', 0):
                    saldo_pendiente = 0
                    factura['estado'] = 'cobrada'
                elif factura['total_abonado'] > 0:
                    factura['estado'] = 'abonada'
                else:
                    factura['estado'] = 'pendiente'
...
def editar_categoria(id):
    ...
    if not nuevo_nombre or not nombre_actual:
        flash('El nuevo nombre y el nombre actual de la categoría son requeridos', 'danger')
        return redirect(url_for('gestionar_categorias'))
    inventario = cargar_datos(ARCHIVO_INVENTARIO)
    ...
    for producto in inventario.values():
        if producto.get('categoria') == nombre_actual:
...
def eliminar_categoria(id):
    ...
    productos_asociados = [p for p in inventario.values() if p.get('categoria') == nombre_actual]
    if len(productos_asociados) > 0:
        flash('No se puede eliminar la categoría porque tiene productos asociados', 'danger')
        return redirect(url_for('gestionar_categorias'))
    # Eliminar productos de la categoría (si existiera alguno sin stock, por ejemplo)
...
def ajustes_masivos():
    ...
    if filtro_categoria:
        ajustes = [a for a in ajustes if a.get('categoria') == filtro_categoria]
    
    # Ordenar
    if orden == 'fecha':
        ajustes.sort(key=lambda a: a['fecha'], reverse=True)
    elif orden == 'producto':
        ajustes.sort(key=lambda a: a['producto'].lower())
    elif orden == 'categoria':
        ajustes.sort(key=lambda a: a.get('categoria', '').lower())
    elif orden == 'stock':
        ajustes.sort(key=lambda a: a['ingreso'] - a['salida'], reverse=True)
...
def actualizar_estados_facturas():
    ...
    for id, factura in facturas.items():
        ...
        total_factura = float(factura.get('total_usd', 0))
        saldo_pendiente = total_factura - total_abonado
        # Redondeo para evitar errores de decimales
        if round(total_abonado, 2) >= round(total_factura, 2) and total_factura > 0:
            factura['estado'] = 'cobrada'
            factura['saldo_pendiente'] = 0
        elif total_abonado > 0 and saldo_pendiente > 0:
            factura['estado'] = 'abonada'
            factura['saldo_pendiente'] = saldo_pendiente
        else:
            factura['estado'] = 'pendiente'
...
def actualizar_estados_facturas():
    ...
    for factura in facturas:
        total_abonado = 0
        for pago in factura.get('pagos', []):
            monto = float(pago.get('monto', 0))
            t_bcv = float(pago.get('tasa_bcv', 0))
            if pago.get('moneda') == 'bs':
                total_abonado += monto / t_bcv if t_bcv else 0
            else:
                total_abonado += monto
